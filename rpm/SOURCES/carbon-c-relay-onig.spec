...
Source0:            https://github.com/msaf1980/carbon-c-relay/archive/%{commit}/%{appname}-%{version}.%{rel}.tbz
...
install -Dp -m0755 relay %{buildroot}%{_bindir}/%{appname}
install -Dp -m0644 %{SOURCE5} %{buildroot}%{_sysconfdir}/%{appname}.conf
...
%if 0%{?rhel} && 0%{?rhel} <= 6
mkdir -p %{buildroot}%{_localstatedir}/log/%{appname}
mkdir -p %{buildroot}%{_localstatedir}/run/%{appname}
mkdir -p %{buildroot}%{_sysconfdir}/logrotate.d
install -Dp -m0755 %{SOURCE2} %{buildroot}%{_initddir}/%{appname}
install -Dp -m0644 %{SOURCE3} %{buildroot}%{_sysconfdir}/logrotate.d/%{appname}
install -Dp -m0644 %{SOURCE4} %{buildroot}%{_sysconfdir}/sysconfig/%{appname}
%else
install -Dp -m0644 %{SOURCE1} %{buildroot}%{_unitdir}/%{appname}.service
install -Dp -m0644 %{SOURCE6} %{buildroot}%{_sysconfdir}/sysconfig/%{appname}
install -Dp -m0644 %{SOURCE7} %{buildroot}%{_sysconfdir}/systemd/system/%{appname}.service.d/limits.conf
%endif
...
%if 0%{?rhel} && 0%{?rhel} <= 6
if [ "$1" -ge "1" ] ; then
    /sbin/service %{appname} condrestart >/dev/null 2>&1 || :
fi
%else
...
%if 0%{?rhel} && 0%{?rhel} <= 6
%{_initddir}/%{appname}
%attr(0755,carbon-c-relay,carbon-c-relay) %dir %{_localstatedir}/log/%{appname}
%attr(0755,carbon-c-relay,carbon-c-relay) %dir %{_localstatedir}/run/%{appname}
%{_sysconfdir}/logrotate.d/%{appname}
%else
%{_unitdir}/%{appname}.service
%dir %{_sysconfdir}/systemd/system/%{appname}.service.d
%config(noreplace) %{_sysconfdir}/systemd/system/%{appname}.service.d/limits.conf
%endif
